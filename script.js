        ...
        const SeamlessActionItem = {
            props: { itemData: Object, displayText: String, textClass: { type: String, default: 'text-sm font-black text-gray-700 truncate' }, showIcon: { type: Boolean, default: false } },
            emits: ['view', 'compare', 'action-click'],
            setup(props, { emit }) {
                ...
                const sparkActiveId = ref(null);
                let sparkTimer = null;
                const triggerSpark = (id) => {
                    sparkActiveId.value = id;
                    if (sparkTimer) clearTimeout(sparkTimer);
                    sparkTimer = setTimeout(() => { sparkActiveId.value = null; }, 400);
                };

                let longPressTimer = null;
                let startX = 0;
                let startY = 0;
                let isMovedForClick = false;

                const options = [
                    { id: 'view', label: '查看細項', icon: 'fa-solid fa-file-lines', offsetX: -75, offsetY: -85, colorClass: 'text-teal-300', bgClass: 'bg-teal-900/90', borderClass: 'border-teal-400 shadow-[0_0_30px_rgba(45,212,191,0.7)]' },
                    { id: 'compare', label: '加入比較', icon: 'fa-solid fa-scale-balanced', offsetX: 75, offsetY: -85, colorClass: 'text-indigo-300', bgClass: 'bg-indigo-900/90', borderClass: 'border-stripe-blurple shadow-[0_0_30px_rgba(99,91,255,0.7)]' }
                ];
                const onPointerDown = (e) => { 
                    ...
                    longPressTimer = setTimeout(() => { 
                        ...
                        // 稍微延遲以觸發 CSS 轉場動畫 (從中心展開)
                        setTimeout(() => { isEntering.value = true; }, 10);
                ...
                const onPointerMove = (e) => { 
                    if (!isOpen.value) { 
                        if (longPressTimer) {
                            ...
                            // 防手震與滾動取消：移動超過 15px 取消長按判定
                            if (dist > 15) { clearTimeout(longPressTimer); longPressTimer = null; isMovedForClick = true; } 
                        } 
                        return; 
                    } 
                    if (e.cancelable) e.preventDefault(); 
                    const currentX = e.clientX; const currentY = e.clientY;
                    const distFromOrigin = Math.hypot(currentX - origin.value.x, currentY - origin.value.y);
                    
                    // 盲區 (Deadzone)：手指在原點 20px 內不作動
                    if (distFromOrigin < 20) {
                        if (activeOptionId.value !== null) { activeOptionId.value = null; if (navigator.vibrate) navigator.vibrate(10); }
                        return;
                    }
                    let closestId = null; let minDist = Infinity;
                    for (const opt of options) {
                        const optCenterX = origin.value.x + opt.offsetX;
                        const optCenterY = origin.value.y + opt.offsetY;
                        const dist = Math.hypot(currentX - optCenterX, currentY - optCenterY);
                        if (dist < minDist) { minDist = dist; closestId = opt.id; }
                    }
                    // 若靠近某個按鈕且未向下大幅拖曳，則鎖定該選項
                    if (minDist < 100 && currentY < origin.value.y + 40) {
                        if (activeOptionId.value !== closestId) {
                            activeOptionId.value = closestId;
                            triggerSpark(closestId);
                ...
                const onPointerUp = (e) => { 
                    if (longPressTimer) { 
                        ...
                        longPressTimer = null; 
                        const dist = Math.hypot(e.clientX - startX, e.clientY - startY);
                        if (dist <= 15) emit('action-click'); 
                    }
                    if (isOpen.value) {
                        if (activeOptionId.value === 'view') { emit('view', props.itemData); } 
                        else if (activeOptionId.value === 'compare') { emit('compare', props.itemData); }
                        isEntering.value = false;
                        setTimeout(() => { isOpen.value = false; activeOptionId.value = null; }, 300);
                    } 
                };

                const globalMove = (e) => { if (isOpen.value) onPointerMove(e); };
                const globalUp = (e) => { if (isOpen.value) onPointerUp(e); };
                
                onMounted(() => { window.addEventListener('pointermove', globalMove, { passive: false }); window.addEventListener('pointerup', globalUp); window.addEventListener('pointercancel', globalUp); }); 
                onUnmounted(() => { window.removeEventListener('pointermove', globalMove); window.removeEventListener('pointerup', globalUp); window.removeEventListener('pointercancel', globalUp); });
            ...
            template: `
            <div class="relative w-full touch-none select-none prevent-select my-1" style="touch-action: none; -webkit-touch-callout: none;" @contextmenu.prevent @pointerdown.stop="onPointerDown" @pointermove="onPointerMove" @pointerup="onPointerUp" @pointercancel="onPointerUp" ref="trackRef">

                <div class="w-full transition-all duration-300 flex items-center" :class="[textClass, {'opacity-50 scale-95': isOpen}]">{{ displayText }} <i v-if="showIcon" class="fa-solid fa-circle-info text-[10px] opacity-70 ml-1.5 shrink-0 mt-0.5"></i></div>
                <teleport to="body">
                    <transition name="modal-fade">
                        <div v-if="isOpen" @contextmenu.prevent @touchmove.prevent @pointermove.prevent class="fixed inset-0 z-[120] touch-none select-none overflow-hidden" :style="{ background: 'radial-gradient(circle at ' + origin.x + 'px ' + origin.y + 'px, rgba(99, 91, 255, 0.15) 0%, rgba(8, 12, 20, 0.95) 70%)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)' }">
                            <!-- 中心按壓點 (花蕊) 磁吸移動效果 -->
                            <div class="absolute w-16 h-16 flex items-center justify-center pointer-events-none transform -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]" :style="{ left: (origin.x + centerOffset.x) + 'px', top: (origin.y + centerOffset.y) + 'px', opacity: isEntering ? 1 : 0, transform: isEntering ? 'translate(-50%, -50%) scale(1)' : 'translate(-50%, -50%) scale(0.5)' }">
                                <!-- 外層數位波紋 -->
                                <div class="absolute inset-3 transition-opacity duration-300 transform rotate-45" :class="activeOptionId ? 'opacity-0' : 'opacity-50'">
                                    <div class="absolute inset-0 rounded-md border border-cyan-400/80 animate-ping" style="animation-duration: 2s;"></div>
                                </div>
                                
                                <!-- 噴泉式煙火小彩球特效 -->
                                <div class="absolute inset-0 transition-all duration-300 pointer-events-none" :class="activeOptionId ? 'opacity-20 scale-75' : 'opacity-100 scale-100'">
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-2 h-2 rounded-sm bg-gradient-to-br from-cyan-300 to-blue-500 shadow-[0_0_8px_rgba(34,211,238,0.8)]" style="--tx: -45px; --ty: -55px; animation-duration: 1.2s; animation-delay: 0s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-2.5 h-2.5 rounded-sm bg-gradient-to-br from-teal-300 to-emerald-500 shadow-[0_0_8px_rgba(45,212,191,0.8)]" style="--tx: 50px; --ty: -45px; animation-duration: 1.5s; animation-delay: 0.2s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-1.5 h-1.5 rounded-sm bg-gradient-to-br from-sky-300 to-blue-600 shadow-[0_0_8px_rgba(56,189,248,0.8)]" style="--tx: -55px; --ty: 15px; animation-duration: 1.3s; animation-delay: 0.4s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-2 h-2 rounded-sm bg-gradient-to-br from-indigo-300 to-purple-500 shadow-[0_0_8px_rgba(129,140,248,0.8)]" style="--tx: 40px; --ty: 45px; animation-duration: 1.4s; animation-delay: 0.1s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-1.5 h-1.5 rounded-sm bg-gradient-to-br from-cyan-200 to-teal-400 shadow-[0_0_6px_rgba(34,211,238,0.8)]" style="--tx: -15px; --ty: 60px; animation-duration: 1.6s; animation-delay: 0.3s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-3 h-3 rounded-sm bg-gradient-to-br from-blue-300 to-indigo-500 shadow-[0_0_6px_rgba(96,165,250,0.8)]" style="--tx: 25px; --ty: -65px; animation-duration: 1.2s; animation-delay: 0.5s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-2 h-2 rounded-sm bg-gradient-to-br from-teal-200 to-cyan-500 shadow-[0_0_8px_rgba(45,212,191,0.8)]" style="--tx: -35px; --ty: -20px; animation-duration: 1.4s; animation-delay: 0.6s;"></div></div>
                                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"><div class="firework-particle w-2 h-2 rounded-sm bg-gradient-to-br from-sky-200 to-blue-400 shadow-[0_0_8px_rgba(125,211,252,0.8)]" style="--tx: 35px; --ty: 10px; animation-duration: 1.3s; animation-delay: 0.7s;"></div></div>
                                </div>

                                <!-- 中心菱形科技節點 -->
                                <div class="w-6 h-6 transition-all duration-300 flex items-center justify-center transform" :class="{'scale-75 opacity-80': activeOptionId}">
                                    <div class="w-full h-full rounded-md bg-gradient-to-br from-cyan-400 to-blue-800 shadow-[0_0_15px_rgba(34,211,238,0.8),inset_0_0_8px_rgba(255,255,255,0.4)] border border-cyan-300/50 backdrop-blur-md flex items-center justify-center animate-spin" style="animation-duration: 4s;">
                                        <div class="w-2 h-2 bg-cyan-100 shadow-[0_0_8px_#fff]"></div>
                                    </div>
                                </div>
                            </div>
                            <div v-for="opt in options" :key="opt.id" class="absolute flex items-center justify-center transform -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] pointer-events-none" :style="{ left: (isEntering ? origin.x + opt.offsetX : origin.x) + 'px', top: (isEntering ? origin.y + opt.offsetY : origin.y) + 'px', transform: activeOptionId === opt.id ? 'translate(-50%, -50%) scale(1.25)' : 'translate(-50%, -50%) scale(1)', opacity: isEntering ? 1 : 0 }">
                                 
                                 <!-- 滑入選項火花噴射特效 -->
                                 <div v-if="sparkActiveId === opt.id" class="absolute inset-0 pointer-events-none z-50">
                                     <div class="spark-particle w-2 h-2 rounded-sm bg-cyan-300 shadow-[0_0_8px_rgba(34,211,238,0.8)]" style="--sx: -35px; --sy: -45px;"></div>
                                     <div class="spark-particle w-2.5 h-2.5 rounded-sm bg-blue-400 shadow-[0_0_8px_rgba(96,165,250,0.8)]" style="--sx: 45px; --sy: -35px;"></div>
                                     <div class="spark-particle w-1.5 h-1.5 rounded-sm bg-teal-200 shadow-[0_0_8px_rgba(153,246,228,0.8)]" style="--sx: -40px; --sy: 15px;"></div>
                                     <div class="spark-particle w-2 h-2 rounded-sm bg-sky-400 shadow-[0_0_8px_rgba(56,189,248,0.8)]" style="--sx: 35px; --sy: 30px;"></div>
                                     <div class="spark-particle w-1.5 h-1.5 rounded-sm bg-cyan-200 shadow-[0_0_8px_rgba(165,243,252,0.8)]" style="--sx: 0px; --sy: -55px;"></div>
                            ...
                            <div class="absolute left-1/2 top-[15%] transform -translate-x-1/2 text-center w-full px-8 pointer-events-none transition-opacity duration-300" :style="{ opacity: isEntering ? 1 : 0 }">
                                ...
                                <p class="text-xs font-bold animate-pulse text-white/70">拖曳至按鈕放開以執行動作</p>
                            </div>
                        </div>
                    </transition>
                </teleport>
        ...
        const GamifiedCompareModal = {
            ...
            setup(props, { emit }) {
                ...
                const showZoneSparks = ref(false); let zoneSparkTimer = null;
                watch(isHoveringZone, (newVal) => {
                    if (newVal) {
                        showZoneSparks.value = true;
                        if (zoneSparkTimer) clearTimeout(zoneSparkTimer);
                        zoneSparkTimer = setTimeout(() => { showZoneSparks.value = false; }, 400);
                ...
                return { dockItems, zoneItems, draggingItem, dragPos, isHoveringZone, showZoneSparks, zoneRef, containerRef, startDrag, onMove, endDrag, removeFromZone, removeFromDock, clearAll, getOrbitStyle, handleClose, analyzeZone, analyzeAll };
            },
            template: `
            <div class="absolute inset-0 z-[120] glass-panel flex flex-col items-center overflow-hidden touch-none select-none prevent-select" ref="containerRef" @pointermove="onMove" @pointerup="endDrag" @pointercancel="endDrag">
                ...
                <div class="flex-1 w-full flex items-center justify-center relative mt-[-10%]">
                    <div ref="zoneRef" class="w-48 h-48 rounded-full border-2 flex flex-col items-center justify-center transition-all duration-300 relative z-10 shadow-sm" :class="isHoveringZone ? 'border-stripe-blurple bg-indigo-50/80 dark:bg-indigo-900/40 neon-glow-purple scale-110' : 'border-indigo-200 dark:border-gray-600 bg-white/50 dark:bg-gray-800/50 border-dashed'">
                        ...
                        <!-- 火花特效 (撞擊戰情分析台) -->
                        <div v-if="showZoneSparks" class="absolute inset-0 pointer-events-none z-30">
                            <style>
                                @keyframes sparkFly {
                                ...
                                .spark-particle {
                                    ...
                                    animation: sparkFly 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;
                                }
                            </style>
                            <div class="spark-particle w-3 h-3 rounded-sm bg-cyan-300 shadow-[0_0_10px_rgba(34,211,238,0.8)]" style="--sx: -80px; --sy: -90px;"></div>
                            <div class="spark-particle w-4 h-4 rounded-sm bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,0.8)]" style="--sx: 90px; --sy: -60px;"></div>
                            <div class="spark-particle w-2.5 h-2.5 rounded-sm bg-teal-400 shadow-[0_0_10px_rgba(45,212,191,0.8)]" style="--sx: -70px; --sy: 60px;"></div>
                            <div class="spark-particle w-3 h-3 rounded-sm bg-sky-400 shadow-[0_0_10px_rgba(56,189,248,0.8)]" style="--sx: 80px; --sy: 80px;"></div>
                            <div class="spark-particle w-4 h-4 rounded-sm bg-indigo-400 shadow-[0_0_10px_rgba(129,140,248,0.8)]" style="--sx: 0px; --sy: -110px;"></div>
                            <div class="spark-particle w-3 h-3 rounded-sm bg-cyan-200 shadow-[0_0_10px_rgba(165,243,252,0.8)]" style="--sx: 0px; --sy: 100px;"></div>
                            <div class="spark-particle w-2 h-2 rounded-sm bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.8)]" style="--sx: -100px; --sy: 0px;"></div>
                            <div class="spark-particle w-3.5 h-3.5 rounded-sm bg-teal-300 shadow-[0_0_10px_rgba(94,234,212,0.8)]" style="--sx: 110px; --sy: 0px;"></div>
                        </div>
                ...
                <div class="w-full bg-white/90 backdrop-blur dark:bg-stripe-darkCard p-5 rounded-t-3xl border-t border-indigo-50 dark:border-gray-700 pb-safe shrink-0 relative z-10 shadow-[0_-10px_40px_rgba(99,91,255,0.06)] dark:shadow-none">
                    ...
                    <div class="flex overflow-x-auto no-scrollbar gap-3 pb-2 min-h-[70px]">
                        <div v-for="item in dockItems" :key="item.id" class="w-16 mt-2 h-16 shrink-0 bg-indigo-50/50 dark:bg-gray-800 rounded-2xl border border-indigo-100/50 dark:border-gray-700 flex flex-col items-center justify-center p-1 cursor-grab active:cursor-grabbing relative transition-all duration-300 touch-none shadow-sm hover:border-stripe-blurple" :class="{'opacity-30 scale-90': draggingItem && draggingItem.id === item.id}" @pointerdown.stop="startDrag($event, item, false)">
        ...
        const app = createApp({
            ...
            setup() {
                ...
                const logoutDragX = ref(0);
                const isLogoutDragging = ref(false);
                const logoutMaxX = 248; // 滑動軌道總長度 (320px) - 按鈕寬度 (60px) - 左右間距 (12px)
                let logoutStartX = 0;

                const startLogoutDrag = (e) => {
                    if (e.pointerType === 'mouse' && e.button !== 0) return;
                    isLogoutDragging.value = true;
                    logoutStartX = e.clientX;
                    document.addEventListener('pointermove', onLogoutDrag, { passive: false });
                    document.addEventListener('pointerup', endLogoutDrag);
                    document.addEventListener('pointercancel', endLogoutDrag);
                    if (navigator.vibrate) navigator.vibrate(10);
                };

                const onLogoutDrag = (e) => {
                    if (!isLogoutDragging.value) return;
                    if (e.cancelable) e.preventDefault();
                    let delta = e.clientX - logoutStartX;
                    if (delta < 0) delta = 0;
                    if (delta > logoutMaxX) delta = logoutMaxX;
                    logoutDragX.value = delta;
                };

                const endLogoutDrag = () => {
                    isLogoutDragging.value = false;
                    document.removeEventListener('pointermove', onLogoutDrag);
                    document.removeEventListener('pointerup', endLogoutDrag);
                    document.removeEventListener('pointercancel', endLogoutDrag);
                    if (logoutDragX.value >= logoutMaxX * 0.85) {
                        if (navigator.vibrate) navigator.vibrate(50);
                        handleLogout(); showLogoutOverlay.value = false;
                    }
                    logoutDragX.value = 0;
                };
                const cancelLogout = () => { showLogoutOverlay.value = false; logoutDragX.value = 0; };
                ...
                const slotDragState = ref({ isDragging: false, item: null, startX: 0, startY: 0, currentX: 0, currentY: 0, width: 0, height: 0 });
                ...
                const onSlotDragStart = (idx, e) => { 
                    ...
                    const targetEl = e.currentTarget.closest('.slot-item');
                    if (!targetEl) return;
                    const rect = targetEl.getBoundingClientRect();
                    
                    // 初始化幽靈元素的狀態
                    slotDragState.value = { isDragging: true, item: compareSlots.value[idx], startX: e.clientX - rect.left, startY: e.clientY - rect.top, currentX: rect.left, currentY: rect.top, width: rect.width, height: rect.height };
                    draggingIndex.value = idx; 
                    if (navigator.vibrate) navigator.vibrate(20);

                    const onMove = (moveEvent) => { 
                        if (!slotDragState.value.isDragging) return; 
                        moveEvent.preventDefault();
                        
                        // 幽靈元素即時跟隨指尖
                        slotDragState.value.currentX = moveEvent.clientX - slotDragState.value.startX;
                        slotDragState.value.currentY = moveEvent.clientY - slotDragState.value.startY;

                        if (Date.now() - lastSwapTime < 100) return; // 縮短延遲至 100ms 讓反應更靈敏
                        const elements = document.elementsFromPoint(moveEvent.clientX, moveEvent.clientY); 
                        // 尋找游標下的目標元素 (避開已經透明化 pointer-events-none 的拖曳佔位符)
                        const dropEl = elements.find(el => el.classList && el.classList.contains('slot-item') && !el.classList.contains('opacity-0')); 
                        if (dropEl) { 
                    ...
                    const onEnd = () => { 
                        ...
                        slotDragState.value.isDragging = false;
                        slotDragState.value.item = null;
                        if (navigator.vibrate) navigator.vibrate(20);
                ...
                return {
                    ...
                    showLogoutOverlay, logoutDragX, isLogoutDragging, logoutMaxX, startLogoutDrag, cancelLogout,
